  ...
  return (
    <div className="relative flex-shrink-0">
      ...
      {/* <output> rather than a div with role="status": it carries the
          status role natively, so assistive tech announces connection
          changes without the role having to be bolted on. `block` because
          <output> is inline by default. */}
      <output
        id="connection-details-panel"
        className={`fixed top-16 left-1/2 z-20 block w-[min(92vw,22rem)] -translate-x-1/2 space-y-2 rounded-md border border-border bg-card/95 p-3 backdrop-blur-sm transition-opacity md:absolute md:top-full md:left-0 md:mt-3 md:w-[24rem] md:translate-x-0 md:translate-y-0 ${
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <div className="text-xs font-semibold tracking-wide text-foreground">Connection Details</div>
          <div className="text-[11px] text-muted-foreground">RTC {connectionState.toLowerCase()}</div>
        </div>
