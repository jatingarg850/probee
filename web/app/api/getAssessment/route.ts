...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[getAssessment] Backend error:', response.status, error)
    ...
    console.error('[getAssessment] Route error:', error)
    return Response.json({ error: 'Could not get the assessment.' }, { status: 500 })
  }
}
