...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/startAgent`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('[startAgent] Backend error:', response.status, error)
    ...
    // Not `error.message`: a `fetch` failure here is a network/DNS error
    // that can name internal hosts, and anything else is unexpected — either
    // way that detail belongs in the server log, not the response body.
    console.error('[startAgent] Route error:', error)
    return Response.json({ error: 'Could not start the interview.' }, { status: 500 })
  }
}
