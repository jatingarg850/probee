...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/matchJobs`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('[matchJobs] Backend error:', response.status, error)
    ...
    console.error('[matchJobs] Route error:', error)
    return Response.json({ error: 'Could not match jobs.' }, { status: 500 })
  }
}
