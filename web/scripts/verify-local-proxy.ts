...
async function withStubBackend<T>(run: (baseUrl: string) => Promise<T>) {
  ...
  // Bun types `Server.port` as optional (a unix-socket server has none), so
  // keep the port we actually bound rather than reading it back.
  let boundPort = 0
  ...
  try {
    return await run(`http://localhost:${boundPort}`)
  } finally {
    server.stop(true)
  }
}
